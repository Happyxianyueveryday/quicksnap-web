...
class User(UserMixin, db.Model):
    ...
    def followed_posts(self):
        '''
        : followed_posts: 查询
        : notes: 我们在这里将查询步骤设置为3步：第一步是将followers表和Post表相连接，得到每个用户所关注的用户的全部微博，第二步是筛选用户，用户的id应该等同于当前登录的用户id，第三步是进行排序，因为显示微博动态时往往均显示最新的微博动态，因此需要按照时间戳进行排序
        '''
...
class Post(db.Model):
    ...
    id = db.Column(db.Integer, primary_key = True)                              # 微博的id，Integer类型，作为表Post的主键，唯一确定一个特定的用户
    body = db.Column(db.String(160))                                            # 微博的内容，String类型
    timestamp = db.Column(db.DateTime, index=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))                   # 微博的创建者/发送者，这里使用的是SQLAlchemy的第一类db.relationship方法，db.Integer指定创建的新属性类型为Integer，db.Foreignkey()指定外键关系，这里创建的user_id参照User表的主键user_id
...
#       3. 由于上述的一对多关系，为了进一步方便访问，在一的一方User中，调用第二类db.relationship方法: posts = db.relationship('Post', backref='author', lazy='dynamic')，该方法配合上面的外键将Post和User连接起来，可以直接访问Post的authot属性，访问发送该Post的用户对象User